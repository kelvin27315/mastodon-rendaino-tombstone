...
def log_in(
    client_file_name: str, api_url: str, mail: str, password: str, user_file_name: str
) -> None:
    """
    アカウントの認証を通す
    """
    mastodon = Mastodon(client_id=PATH + client_file_name, api_base_url=api_url)
    mastodon.log_in(
        mail, password, scopes=["read", "write", "follow"], to_file=PATH + user_file_name
    )
